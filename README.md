...
The container is deployed to an AWS EC2 instance.

Nginx is used as a reverse proxy to route incoming requests.

End users access the application via the cloud.
...
Code is pushed to the feature branch.
...
The application is prepared for deployment on AWS.
...
Configure security groups to allow HTTP and SSH.
...
Configure Nginx reverse proxy for routing.
